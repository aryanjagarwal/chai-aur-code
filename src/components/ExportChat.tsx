...
const ExportChat = ({ messages, isOpen, onClose }: ExportChatProps) => {
  ...
  const formatMessages = (format: ExportFormat) => {
    ...
    switch (format) {
      case 'txt':
        return filteredMessages.map(msg => {
          const timestamp = includeTimestamps 
            ...
            : '';
          const sender = msg.isUser ? 'You' : 'Hitesh';
      ...
      case 'md':
        return filteredMessages.map(msg => {
          const timestamp = includeTimestamps 
            ? `*${msg.timestamp.toLocaleString()}*\n` 
            : '';
          const sender = msg.isUser ? '**You**' : '**Hitesh Choudhary**';
          let content = msg.content;
      ...
      case 'json':
        return JSON.stringify(
          filteredMessages.map(msg => ({
            ...
            content: includeCodeBlocks ? msg.content : msg.content.replace(/```[\s\S]*?```/g, '[Code Block Removed]'),
      ...
      case 'html':
        const htmlMessages = filteredMessages.map(msg => {
          const timestamp = includeTimestamps 
          ...
          if (!includeCodeBlocks) {
            content = content.replace(/```[\s\S]*?```/g, '<em>[Code Block Removed]</em>');
...
<head>
    ...
    <title>Chat Export - Hitesh Choudhary AI</title>
    <style>
...
<body>
    <h1>Chat Export - Hitesh Choudhary AI</h1>
  ...
  const handleExport = () => {
    ...
    const filename = `hitesh-chat-${timestamp}.${format}`;
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Chat with Hitesh Choudhary AI',
          text: content,
        });
      } catch (err) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Export your conversation with Hitesh Choudhary AI in various formats.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox 
                  ...
                  onCheckedChange={setIncludeCodeBlocks}
                />
              ...
              <div className="flex items-center space-x-2">
                <Checkbox 
                  ...
                  onCheckedChange={setUserMessagesOnly}
                />
          ...
          <div className="bg-muted/30 rounded-lg p-3 text-sm text-muted-foreground">
            <div className="flex justify-between">
              ...
              <span>{userMessagesOnly ? messages.filter(m => m.isUser).length : messages.length}</span>
            </div>
            <div className="flex justify-between">
              <span>Date range:</span>
              <span>
                {messages.length > 0 && 
                  `${messages[messages.length - 1].timestamp.toLocaleDateString()} - ${messages[0].timestamp.toLocaleDateString()}`
                }
              </span>
            </div>
          </div>
        ...
        <DialogFooter className="flex-col sm:flex-row gap-2">
          ...
          <Button variant="outline" onClick={shareChat} className="w-full sm:w-auto">
